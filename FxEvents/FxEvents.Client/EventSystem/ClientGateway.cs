    ...
    public class ClientGateway : BaseGateway
    {
        ...
        public ClientGateway()
        {
            ...
            EventDispatcher.Instance.AddEventHandler(EventConstant.InboundPipeline, new Action<byte[]>(async serialized =>
            {
                try
                {
                    await ProcessInboundAsync(-1, serialized);
                }
                catch (Exception ex)
                {
        ...
        {
            if (source != -1) throw new Exception($"The client can only target server events. (arg {source} is not matching -1)");
        ...
        {
            await SendInternal(EventFlowType.Straight, -1, endpoint, args);
        }
