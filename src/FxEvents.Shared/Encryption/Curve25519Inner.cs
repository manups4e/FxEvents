...
{
	internal class Curve25519Inner
	{
		/* key size */
		...
		public static void ClampPrivateKeyInline(byte[] key)
		{
			...
			if (key.Length != 32)
				throw new ArgumentException(
					String.Format("key must be 32 bytes long (but was {0} bytes long)", key.Length)
				);
		...
		public static byte[] ClampPrivateKey(byte[] rawKey)
		{
			...
			if (rawKey.Length != 32)
				throw new ArgumentException(
					String.Format(
						"rawKey must be 32 bytes long (but was {0} bytes long)",
						rawKey.Length
					),
					"rawKey"
				);
		...
		static void Multiply(Long10 xy, Long10 x, Long10 y)
		{
			...
			t =
				(x0 * y0)
				+ 19 * ((t >> 25) + (x2 * y8) + (x4 * y6) + (x6 * y4) + (x8 * y2))
		...
		)
		{
			...
			if (signingKey != null && signingKey.Length != 32)
				throw new ArgumentException(
					String.Format(
						"signingKey must be null or 32 bytes long (but was {0} bytes long)",
						signingKey.Length
					),
					"signingKey"
				);
			...
			if (peerPublicKey != null && peerPublicKey.Length != 32)
				throw new ArgumentException(
					String.Format(
						"peerPublicKey must be null or 32 bytes long (but was {0} bytes long)",
